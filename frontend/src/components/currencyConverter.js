  ...
  return (
    <Autocomplete
      ...
      isOptionEqualToValue={
        (option, value) => option.currCode === value.currCode
      }
...
const CurrencyConverter = ({rates}) => {
  const currencyRates = Object.keys(rates.rates).sort().map(
      (item) => {
  ...
  const getConversion = useCallback( () => {
    setConverted(
        convertCurrency(
            rates.rates[toCode.currCode],
            rates.rates[fromCode.currCode],
            amountDebounced,
        ),
    );
    ...
    <Stack
      ...
      <Box display="flex" justifyContent="center" alignItems="center">
        <Typography variant="h6" gutterBottom>
          {currencyFormatter(amountDebounced, fromCode.currCode)}
          {' = '}
          {currencyFormatter(converted, toCode.currCode)}
        </Typography>
      </Box>
      <Box
        component="form"
        ...
        <TextField
          ...
          type="number"
          inputProps={{
            min: 0, style: {textAlign: 'center', fontSize: 25, padding: 1},
          }}
          value={amount}
          onChange={
            (event) => setAmount(event.target.value? event.target.value: 1)
          }
